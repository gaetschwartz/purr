...
use crate::error::{AudioFormatError, WebError, WebResult};
...
impl AudioTranscriptionProcessor {
    ...
    fn parse_wav_metadata(&self, data: &[u8]) -> WebResult<AudioMetadata> {
        ...
        let fmt_chunk =
            fmt_data.ok_or_else(|| WebError::from(AudioFormatError::NoFmtChunkFound))?;
        ...
        let channels = u32::from(u16::from_le_bytes([fmt_chunk[2], fmt_chunk[3]]));
        let sample_rate =
            u32::from_le_bytes([fmt_chunk[4], fmt_chunk[5], fmt_chunk[6], fmt_chunk[7]]) as f32;
            ...
            if data[offset] == 0xFF && (data[offset + 1] & 0xE0) == 0xE0 {
                ...
                if samples_per_frame > 0 {
                    ...
                    if (bitrate_index as usize) < bitrates.len()
                        && (sampling_freq_index as usize) < sample_rates.len()
                    {
    ...
    fn parse_flac_metadata(&self, data: &[u8]) -> WebResult<AudioMetadata> {
        ...
        let channels = u32::from(((streaminfo[12] & 0x0E) >> 1) + 1);
        ...
        Ok(AudioMetadata {
            ...
            duration,
            file_size: data.len(),
            bit_depth: Some(u16::from(bits_per_sample)),
    ...
    pub fn convert_raw_audio_data(
        ...
        match metadata.format {
            ...
            _ => {
                ...
                tracing::info!(
                    "Audio format {} will be processed by worker.js",
                    metadata.format.as_str()
                );
    ...
    fn convert_wav_to_samples(&self, data: &[u8], metadata: &AudioMetadata) -> WebResult<Vec<f32>> {
        ...
        let pcm_data =
            data_chunk.ok_or_else(|| WebError::from(AudioFormatError::NoDataChunkFound))?;
        ...
        let samples = match bit_depth {
            ...
            16 => {
                ...
                for chunk in pcm_data.chunks_exact(2) {
                    ...
                    samples.push(f32::from(sample16) / 32768.0);
                }
                samples
            }
            24 => {
    ...
    fn parse_m4a_channels(&self, data: &[u8]) -> WebResult<u32> {
        ...
        if let Some(stsd_pos) = self.find_mp4_atom(data, b"stsd") {
            if data.len() > stsd_pos + 20 {
                return Ok(u32::from(u16::from_be_bytes([
                    data[stsd_pos + 16],
                    data[stsd_pos + 17],
                ])));
            }
    ...
    fn parse_webm_sample_rate(&self, data: &[u8]) -> WebResult<f32> {
        ...
        if let Some(pos) = self.find_webm_element(data, &[0x86]) {
            // SamplingFrequency
            if data.len() > pos + 8 {
                // Parse float64 sampling frequency
                let freq_bytes = &data[pos + 1..pos + 9];
                let freq = f64::from_be_bytes([
                    freq_bytes[0],
                    freq_bytes[1],
                    freq_bytes[2],
                    freq_bytes[3],
    ...
    fn parse_webm_channels(&self, data: &[u8]) -> WebResult<u32> {
        ...
        if let Some(pos) = self.find_webm_element(data, &[0x9F]) {
            // Channels
            if data.len() > pos + 2 {
                return Ok(u32::from(data[pos + 1]));
            }
        }
        Err(WebError::UnsupportedAudioFormat {
            format: "webm".to_string(),
    ...
    fn find_webm_element(&self, data: &[u8], element_id: &[u8]) -> Option<usize> {
        (0..data.len().saturating_sub(element_id.len()))
            .find(|&i| &data[i..i + element_id.len()] == element_id)
    }
...
pub async fn start_transcription_process(
    ...
    let processed_samples =
        processor.convert_audio_data(&file_data, &metadata, move |progress| match progress {
    ...
    // Start transcription
    let transcription_stream = processor
        .worker
...
pub fn get_supported_formats() -> HashMap<String, Vec<String>> {
    ...
    formats.insert(
        "wav".to_string(),
        vec!["PCM".to_string(), "IEEE Float".to_string()],
    );
    ...
    formats.insert(
        "webm".to_string(),
        vec!["Opus".to_string(), "Vorbis".to_string()],
    );
