    ...
    match extension.to_lowercase().as_str() {
        "wav" | "mp3" | "ogg" | "flac" => {
            ...
            cmd.arg(sample_path)
                ...
                .arg("--output").arg("json") // Use JSON for easier validation
            ...
            if output.status.success() {
                ...
                if let Ok(json_value) = serde_json::from_str::<serde_json::Value>(&stdout) {
                    ...
                    assert!(json_value.get("text").is_some(), 
                           "JSON output should have 'text' field for {}", sample_path.display());
                    assert!(json_value.get("segments").is_some(), 
                           "JSON output should have 'segments' field for {}", sample_path.display());
                    assert!(json_value.get("audio_duration").is_some(), 
                           "JSON output should have 'audio_duration' field for {}", sample_path.display());
                    
                    let text = json_value["text"].as_str().unwrap_or("");
                    assert!(!text.is_empty(), 
                           "Transcribed text should not be empty for {}", sample_path.display());
                    
                    println!("✓ CLI successfully transcribed {}: \"{}...\"", 
                            sample_path.file_name().unwrap().to_string_lossy(),
                            text.chars().take(50).collect::<String>());
                } else {
                    panic!("Failed to parse JSON output for {}: {}", sample_path.display(), stdout);
                }
            } else {
                ...
                if stderr.contains("No Whisper model found") || stderr.contains("No model available") {
                    println!("⚠ Skipping CLI test for {} - no model available", sample_path.display());
                    return;
                } else {
                    panic!("CLI transcription failed for {}: {}", sample_path.display(), stderr);
                }
            }
        },
        "md" => {
        ...
        _ => {
            println!("⚠ Unknown file type for CLI test: {} (extension: {})", sample_path.display(), extension);
            return;
        }
    }
}
    ...
    if output.status.success() {
        ...
        match format {
            "json" => {
                ...
                assert!(json_result.is_ok(), "Should produce valid JSON for format: {}", format);
                println!("✓ CLI JSON format test successful");
            },
            "srt" => {
                // Check for SRT format patterns (timestamps)
                assert!(stdout.contains("-->"), "SRT format should contain timestamp arrows");
                println!("✓ CLI SRT format test successful");
            },
            "text" => {
                // Text should be readable
                assert!(stdout.chars().any(|c| c.is_alphabetic()), 
        ...
        let stderr = String::from_utf8(output.stderr).unwrap();
        if stderr.contains("No Whisper model found") || stderr.contains("No model available") {
            println!("⚠ Skipping CLI output format test for {} - no model available", format);
            return;
        } else {
...
fn test_cli_model_commands() {
    ...
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Models list command should succeed");
    
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("Available Whisper Models"), 
    ...
    // Test downloaded models command
    let mut cmd = Command::cargo_bin("whisper-ui-cli").unwrap();
    cmd.arg("models").arg("downloaded");
    
    let output = cmd.output().unwrap();
    assert!(output.status.success(), "Models downloaded command should succeed");
    
    println!("✓ CLI models downloaded command successful");
}
...
fn test_cli_error_handling(#[case] invalid_path: &str, #[case] description: &str) {
    ...
    let output = cmd.output().unwrap();
    assert!(!output.status.success(), "{}: {}", description, invalid_path);
    
    let stderr = String::from_utf8(output.stderr).unwrap();
    assert!(!stderr.is_empty(), "Should produce error message for: {}", invalid_path);
    
    println!("✓ CLI correctly handled invalid file: {} -> {}", invalid_path, 
...
fn test_cli_verbose_mode() {
    ...
    if output.status.success() {
        ...
        assert!(stderr.contains("Whisper UI") || stderr.contains("Audio file") || stderr.contains("GPU acceleration"),
               "Verbose mode should produce additional output");
